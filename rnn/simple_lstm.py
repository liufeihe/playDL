...
class LstmNetwork():
    ...
    def y_list_is(self, y_list, loss_layer):
        ...
        # ... following nodes also get diffs from next nodes, hence wwe add diffs to diff_h
...
def example_0():
    ...
    np.random.seed(0)
    # parameters for input data dimension and lstm cell output
    mem_cell_ct = 100
    x_dim = 50
    lstm_param = LstmParam(mem_cell_ct, x_dim)
    lstm_net = LstmNetwork(lstm_param)
    ...
    for cur_iter in range(1000):
        ...
        if cur_iter %10 == 0:
            ...
            print "y_pred= [" + ', '.join(["%2.5f" % lstm_net.lstm_node_list[ind].state.h[0] for ind in range(len(y_list))]) + ']'
            print 'loss: %.3e'%loss
